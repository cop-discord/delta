...
class Premium(commands.Cog):
   ...
   async def perks(self, ctx: commands.Context): 
      embed = discord.Embed(color=self.bot.color, title="", description=">>> **You can acces donor command for boosting our [support server](https://discord.gg/vvdTdmaebh) or with 2$ payment**")
   ...
   async def donate(self, ctx: commands.Context): 
    embed = discord.Embed(color=self.bot.color, title="", description=f">>> **Click below**")
    ...
    async def boost_callback(interaction: discord.Interaction): 
      await interaction.response.send_message("boost https://discord.gg/vvdTdmaebh for perks", ephemeral=True)
